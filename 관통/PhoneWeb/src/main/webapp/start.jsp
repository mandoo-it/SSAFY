...
function phoneList() {//REST 서버에 모든 고객정보에 대한 ajax 요청 보내기
	$.ajax({
		url:'http://localhost:9090/phoneswagger/phone',
		type:'get',
...
function phoneSelect() {
	$('body').on('click','#btnSelect',function(){
		...
		$.ajax({
			url: 'http://localhost:9090/phoneswagger/phone/'+num,
			type: 'get',
			dataType: 'json',
...
function login() {
	$('body').on('click','#btnLogin',function(){
		...
		var pw=$('#pw').val();
		$.ajax({
			url: 'http://localhost:9090/phoneswagger/login/'+id,
			type: 'get',
			dataType: 'json',
			success: function(data){
				loginResult(data);
...
function logout(){
	$('body').on('click', '#btnLogout', function(){
		...
		$.ajax({
			url: 'http://localhost:9090/phoneswagger/logout/'+id,
			type: 'get',
			success: function(data){
				logoutResult(data);
			},
