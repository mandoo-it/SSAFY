...
<script type="text/javascript">
	$(document).ready(function() {
		...
		customerUpdate();//한 사람 저오 수정시 처리한 이벤트 등록
		customerInsert();//새 고객 정보 추가시 처리할 이벤트 등록
		init();
	});
	
	//모든 사용자 목록 조회 요청
	function customerList() {//rest 서버에 모든 고객 정보에 대한 ajax 요청 보내기	
		$.ajax({
			url : 'customers',
			type : 'get',
			dataType : 'json',
			...
			error : function(xhr, status, msg) {
				alert("상태값 : " + status + "http 에라 메세지" + msg);
			}
			
		});
	}
	
	function customerListResult(data) {//rest 서버에 모든 고객 정보에 대한 ajax 요청 보내기	
		$('tbody').empty();
	...
	function customerDelete() {
		$('body').on('click', '#btnDelete', function(){
			var num = $(this).closest('tr').find('#hidden_num').val();
			var result = confirm(num + "사용자를 정말로 삭제하시겠습니까?");
			if(result) {
				$.ajax({
					url : 'customers/' + num,
					type : 'delete',
